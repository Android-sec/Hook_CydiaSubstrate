		...
		if ( hookstring != null )
		{
			...
			MS.hookMethod( StringBuilder_class, hookstring, new MS.MethodHook( )
			{
				...
				public Object invoked( Object arg0, Object... arg1 ) throws Throwable
				{
					...
					if(str.length()<1023){
						Log.i( "hook_StringBuilder_toString", str );
					}else{						
						Log.i( "hook_StringBuilder_toString_Multi-section", "start" );
						int length= 1023;
						int i = 0;
						for(;i<str.length()&& length < str.length() ;i+=1023,length+=1023){
							Log.i( "hook_StringBuilder_toString", str.substring(i,length) );
						}
						Log.i( "hook_StringBuilder_toString", str.substring(i) );
						Log.i( "hook_StringBuilder_toString_Multi-section", "end" );
					}
					return str;
				}
			}, old );
